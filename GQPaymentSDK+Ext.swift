//
//  GQPaymentSDK+Ext.swift
//  GQPaymentIOSSDK
//
//  Created by valentine on 30/01/24.
//
...
public class GQPaymentSDK {
    ...
    weak var gqWebViewController: UIViewController?
    ...
    public init(delegate: GQPaymentDelegate?, displayController: UIViewController?, clientJSONObject: [String : Any]?) {
        self.delegate = delegate
        self.environment = Environment.shared
        self.displayController = displayController
        self.clientJSONObject = clientJSONObject
//        self.prefillJSONObject = prefillJSONObject
//        initialiseSDK()
    }
    ...
    private func getURL(){
        ...
        self.gqWebViewController = gqWebView
        DispatchQueue.main.async {
            self.displayController?.present(gqWebView, animated: true, completion: nil)
        }
...
extension GQPaymentSDK: WebDelegate {
    ...
    func sdCancel(data: [String : Any]?) {
        ...
        DispatchQueue.main.async {
            self.gqWebViewController?.dismiss(animated: false, completion: nil)
        }
